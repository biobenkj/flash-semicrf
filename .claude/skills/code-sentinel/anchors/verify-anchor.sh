...
if [[ -n "$after_pattern" ]]; then
  ...
  after_line=$(grep -n "$after_pattern" "$file" 2>/dev/null | head -1 | cut -d: -f1)
  if [[ -z "$after_line" ]]; then
    echo "ANCHOR_MISSING: Context pattern '$after_pattern' not found"
    exit 1
  fi
  # Search only after the context line
  actual_line=$(tail -n "+$after_line" "$file" | grep -n "$pattern" | head -1 | cut -d: -f1)
  ...
  actual_line=$(grep -n "$pattern" "$file" 2>/dev/null | head -1 | cut -d: -f1)
fi
