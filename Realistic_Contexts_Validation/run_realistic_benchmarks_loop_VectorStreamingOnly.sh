...
OUTDIR=/varidata/research/projects/jang/TommyGoralski/torch_semimarkov/results/realistic/human_exonK_from_stats_sweep_VectorStreamingOnly
mkdir -p "$OUTDIR"
...
python ./benchmarks/benchmark_memory_analysis.py \
  ...
  --B 4 \
  --repeats 3 \
  --backends linear_scan_vectorized,linear_scan_streaming \
  --output-dir "${OUTDIR}"
