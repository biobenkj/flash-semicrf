...
class OneHotEncoder:
    ...
    def __init__(self):
        ...
        #
        #   encoded = torch.zeros(T, dim)
        #   for i, base in enumerate(sequence):
        #       encoded[i, vocab.get(base.upper(), 4)] = 1.0
        #
        # which is a Python loop over T positions. For T ~ 10k–50k and thousands
        # of sequences, that becomes a major CPU bottleneck.
        #
        # Here we build a simple ASCII lookup table (LUT) so encoding becomes:
        #   bytes -> uint8 -> LUT -> indices -> one-hot in a vectorized way.
        lut = np.full(256, 4, dtype=np.int64)  # default to 'N' index
        for base, idx in self.DNA_VOCAB.items():
            lut[ord(base)] = idx
            lut[ord(base.lower())] = idx
        self._lut = lut
    ...
    def encode(self, sequence: str) -> torch.Tensor:
        ...
        Performance notes:
        - Avoids Python-level per-base loops.
        - Uses an ASCII LUT to map characters -> indices.
        - Builds the one-hot tensor via indexed scatter.
        """
        # bytes -> uint8 array
        arr = np.frombuffer(sequence.encode("ascii", "replace"), dtype=np.uint8)
        # map bytes to indices in [0..4]
        idx = self._lut[arr]  # (T,)

        T = idx.shape[0]
        encoded = torch.zeros((T, self.dim), dtype=torch.float32)
        encoded[torch.arange(T), torch.from_numpy(idx)] = 1.0
        return encoded
...
class SyntheticGenomicsDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict[str, Tensor]:
        ...
        # Encode features (now vectorized in OneHotEncoder)
...
def train_epoch(
    ...
    Args:
        ...
        debug_timing: If True, log load/H2D/step timings every ~50 batches.
    """
    model.train()
    total_loss = 0.0
    num_batches = 0

    # For optional micro-timing
    t_prev = time.time()

    for batch_idx, batch in enumerate(dataloader):
        t_loaded = time.time()

        # Non-blocking H2D transfers (works with pin_memory=True)
        ...
        if debug_timing and (batch_idx % 50 == 0):
            # load: dataloader / CPU work between last step and this batch
            # h2d : host->device copy time
            # step: forward + backward + optimizer
            logger.info(
                f"[timing] batch={batch_idx} "
                f"load={t_loaded - t_prev:.3f}s "
                f"h2d={t_h2d - t_loaded:.3f}s "
                f"step={t_step - t_h2d:.3f}s"
            )
...
def train_model(
    ...
    Args:
        ...
        num_workers: DataLoader workers (0 = main process only; if 0 and CUDA is used,
                     a conservative default is chosen).
        prefetch_factor: DataLoader prefetch_factor (only used when num_workers > 0).
        pin_memory: Whether to use pinned memory in DataLoader (helps H2D).
        debug_timing: If True, enable micro-timing logs in train_epoch.
    """
    ...
    # Auto-pick a reasonable default for num_workers if not set and on CUDA
    if num_workers == 0 and torch.cuda.is_available() and device_obj.type == "cuda":
        cpu_ct = os.cpu_count() or 4
        num_workers = min(8, max(1, cpu_ct // 2))

    pin = pin_memory and torch.cuda.is_available() and device_obj.type == "cuda"
...
def main():
    ...
    # New perf-related CLI options
    run_parser.add_argument(
        "--num-workers",
        type=int,
        default=0,
        help="DataLoader worker processes. 0 = main process only; if 0 and running on CUDA, "
        "a conservative default is chosen based on available CPUs.",
    )
    run_parser.add_argument(
        "--prefetch-factor",
        type=int,
    ...
    run_parser.add_argument(
        "--no-pin-memory",
        action="store_true",
        help="Disable pin_memory in DataLoaders (normally helps H2D transfer).",
    )
    run_parser.add_argument(
        "--debug-timing",
        action="store_true",
        help="Log load/h2d/step timings every ~50 batches to diagnose bottlenecks.",
    )
