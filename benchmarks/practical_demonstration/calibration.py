...
class BoundaryUncertaintyData:
    ...
    position_probs: np.ndarray | None = (
        None  # (T, C) position-level class probs (for linear CRF comparison)
    )
...
def compute_mce(
    ...
    """
    _, _, bin_accuracies, bin_confidences, bin_counts = compute_ece(confidences, accuracies, n_bins)
...
def evaluate_confidence_intervals(
    ...
    for boundary_probs, true_boundaries in zip(
        boundary_probs_list, true_boundaries_list, strict=False
    ):
...
def derive_boundary_probs_from_positions(
    ...
    if method == "transition":
        ...
        for t in range(1, T):
            boundary_probs[t] = np.abs(position_probs[t].max() - position_probs[t - 1].max())
    else:
...
class CalibrationEvaluator:
    ...
    def evaluate(
        ...
        # Compute ECE
        ece, bins, bin_acc, bin_conf, bin_counts = compute_ece(all_probs, all_true, self.n_bins)
...
def plot_calibration_comparison(
    ...
    ax.plot(
        ...
        "o-",
        color="#DC267F",
        label=f"Semi-CRF (ECE={semicrf_results.ece:.3f})",
        markersize=6,
    )
    ...
    ax.plot(
        ...
        "s-",
        color="#648FFF",
        label=f"Linear CRF (ECE={linear_results.ece:.3f})",
        markersize=6,
    )
    ...
    ax.plot(
        ...
        "-",
        color="#DC267F",
        label=f"Semi-CRF (AUC={semicrf_results.auc_selective:.3f})",
        linewidth=2,
    )
    ax.plot(
        ...
        "-",
        color="#648FFF",
        label=f"Linear CRF (AUC={linear_results.auc_selective:.3f})",
        linewidth=2,
    )
    ...
    ax.bar(x - width / 2, semicrf_coverage, width, label="Semi-CRF", color="#DC267F")
    ax.bar(x + width / 2, linear_coverage, width, label="Linear CRF", color="#648FFF")
    ...
    ax.set_xticks(x)
    ax.set_xticklabels([f"{level}%" for level in ci_levels])
    ax.legend()
    ax.set_ylim([0, 1.1])
...
def print_calibration_comparison(
    ...
    for level, (s_cov, s_width, l_cov, l_width) in [
        (
            ...
            (
                semicrf_results.ci_coverage_50,
                semicrf_results.ci_width_50,
                linear_results.ci_coverage_50,
                linear_results.ci_width_50,
            ),
        ),
        (
            90,
            (
                semicrf_results.ci_coverage_90,
                semicrf_results.ci_width_90,
                linear_results.ci_coverage_90,
                linear_results.ci_width_90,
            ),
        ),
        (
            95,
            (
                semicrf_results.ci_coverage_95,
                semicrf_results.ci_width_95,
                linear_results.ci_coverage_95,
                linear_results.ci_width_95,
            ),
        ),
    ]:
        nominal = level / 100
