...
This supplement documents the specialized $K=1$ (linear CRF) and $K=2$ (near-linear CRF) implementations in \texttt{torch-semimarkov}. These optimized paths eliminate ring buffer overhead for common sequence labeling tasks while maintaining compatibility with the streaming Semi-CRF interface.
