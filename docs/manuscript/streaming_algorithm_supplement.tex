...
\noindent where $t$ is the segment end position (1-indexed), $k \in \{1, \ldots, K\}$ is the duration, $c$ is the destination state, and $c'$ is the source state. The optional boundary projections $\mathcal{P}^{\text{start}}$ and $\mathcal{P}^{\text{end}}$ score segment start/end positions (e.g., for detecting boundary-specific features). The prefix-sum decomposition allows $O(1)$ edge computation instead of $O(k)$.
...
\For{$t \gets 1$ \KwTo $T$}{
    ...
    \If(\tcp*[f]{Normalize at checkpoint~\cite{dao2022flashattention}}){$t \bmod \Delta = 0$}{
        ...
        $s \gets \begin{cases} \max_c \mathbf{v}_t(c) & \text{if } t \le L_b \\ 0 & \text{otherwise} \end{cases}$\;
